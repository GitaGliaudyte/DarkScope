...
import { getConfidence, matchesDiscouragementPattern, scoreSignals } from './scoring';

interface DiscouragementHit {
  selector: string;
  element: HTMLElement;
  score: number;
  text: string;
...
function isDiscouragementCandidate(element: NormalizedElement): boolean {
  return matchesDiscouragementPattern(element.text);
}

export function findDiscouragementCandidates(snapshot: AnalysisContext['snapshot']): NormalizedElement[] {
  const snapshotCandidates = snapshot.elements.filter(isDiscouragementCandidate);
  const liveTextCandidates = Array.from(document.querySelectorAll<HTMLElement>(LIVE_TEXT_SELECTOR))
    ...
    .filter((element) => matchesDiscouragementPattern(element.text));
...
export function detectDiscouragement(context: AnalysisContext): RuleResult {
  const candidates = findDiscouragementCandidates(context.snapshot);
  const evidence: RuleResult['evidence'] = [];
  const selectors = new Set<string>();
  let highestScore = 0;
  const hits: DiscouragementHit[] = [];
  ...
  for (const hit of prunedHits) {
    ...
    evidence.push({
      ...
      text: hit.text,
      reason: `Discouragement/refusal signal scored ${hit.score}/10`,
